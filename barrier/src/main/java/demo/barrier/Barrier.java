...
public class Barrier {
    ...
    private static final int NUMBER_OF_SERVICE_INSTANCES =
            parseInt(getenv("NUMBER_OF_SERVICE_INSTANCES"));
    ...
    public static void main(String[] args) throws Exception {
        ...
        final DistributedDoubleBarrier barrier =
                new DistributedDoubleBarrier(
                        client,
                        format(zkNodePath, groupName, NUMBER_OF_SERVICE_INSTANCES),
    ...
    private static void waitForOtherClients(
            final CuratorFramework client, final DistributedDoubleBarrier barrier)
            throws Exception {
        ...
        barrier.enter();
        log.info("Node:" + INSTANCE_ID + " entered first barrier.");
        // Do some work after entering barrier
        Thread.sleep(random.nextInt(5000) + 1000);
        barrier.leave();
        log.info("Node:" + INSTANCE_ID + " left the second barrier.");
    }
}
