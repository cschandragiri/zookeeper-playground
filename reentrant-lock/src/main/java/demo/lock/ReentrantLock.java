...
public class ReentrantLock {
    ...
    public static void main(String[] args) throws Exception {
        ...
        final InterProcessLock interProcessLock =
                new InterProcessSemaphoreMutex(client, zkNodePath);
        client.start();
        while (true) {
    ...
    private static void lockAndDoWork(
            final CuratorFramework client, final InterProcessLock interProcessLock)
        ...
        log.info("Node:" + INSTANCE_ID + " attempting to acquire lock.");
        try {
            interProcessLock.acquire();
        } catch (Exception e) {
            log.info("Node:" + INSTANCE_ID + " got error while acquiring lock");
            throw new RuntimeException("Error while acquiring lock", e);
        }
        log.info("Node:" + INSTANCE_ID + " got the shared lock.");
        // Do some work after acquiring lock
        Thread.sleep(random.nextInt(10000) + 1000);

        log.info("Node:" + INSTANCE_ID + " releasing the lock.");
        try {
            interProcessLock.release();
        } catch (Exception e) {
            log.info("Node:" + INSTANCE_ID + " got error while releasing lock");
